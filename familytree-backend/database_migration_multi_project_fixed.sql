...
-- Step 1: 先刪除可能存在的 projects 表 (如果之前建立失敗)
DROP TABLE IF EXISTS projects CASCADE;

-- Step 2: 創建專案管理表 (修正索引語法)
CREATE TABLE projects (
    id VARCHAR(25) PRIMARY KEY,  -- 格式: userID-YYYYMMDDHHMMSS (增加長度)
...
ALTER TABLE person_profile ADD COLUMN IF NOT EXISTS project_id VARCHAR(25);

-- 3.2 analysis_results 表
ALTER TABLE analysis_results ADD COLUMN IF NOT EXISTS project_id VARCHAR(25);

-- 3.3 analysis_sessions 表
ALTER TABLE analysis_sessions ADD COLUMN IF NOT EXISTS project_id VARCHAR(25);

-- 3.4 relationship_layers 表
ALTER TABLE relationship_layers ADD COLUMN IF NOT EXISTS project_id VARCHAR(25);

-- 3.5 missing_persons 表
ALTER TABLE missing_persons ADD COLUMN IF NOT EXISTS project_id VARCHAR(25);

-- 3.6 user_update_file 表
ALTER TABLE user_update_file ADD COLUMN IF NOT EXISTS project_id VARCHAR(25);

-- 3.7 field_mapping 表
ALTER TABLE field_mapping ADD COLUMN IF NOT EXISTS project_id VARCHAR(25);

-- 3.8 search_keywords 表
ALTER TABLE search_keywords ADD COLUMN IF NOT EXISTS project_id VARCHAR(25);

-- 3.9 user_favorites 表
ALTER TABLE user_favorites ADD COLUMN IF NOT EXISTS project_id VARCHAR(25);

-- 3.10 search_logs 表
ALTER TABLE search_logs ADD COLUMN IF NOT EXISTS project_id VARCHAR(25);

-- Step 4: 生成預設專案資料並更新現有資料
DO $$
DECLARE
    default_user_id VARCHAR(6) := LPAD(FLOOR(RANDOM() * 1000000)::TEXT, 6, '0');  -- 生成6位隨機數字
    default_project_id VARCHAR(25);
...
BEGIN
    ...
    RAISE NOTICE '準備建立預設專案: ID = %, User ID = %', default_project_id, default_user_id;
...
BEGIN
    ...
    RAISE NOTICE '✅ 已設定必填欄位約束';
    RAISE NOTICE '==========================================';
    RAISE NOTICE '請檢查以下資訊:';
    RAISE NOTICE '1. 預設專案資料是否正確建立';
    RAISE NOTICE '2. 現有資料是否都已分配到預設專案';
    RAISE NOTICE '3. 外鍵約束是否正常運作';
    RAISE NOTICE '==========================================';
END $$; 