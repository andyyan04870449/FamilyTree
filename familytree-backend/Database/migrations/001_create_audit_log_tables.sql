...
-- 上個月分區
CREATE TABLE audit_logs_previous PARTITION OF audit_logs
...
GRANT SELECT, INSERT, UPDATE, DELETE ON ALL TABLES IN SCHEMA public TO audit_admin;
GRANT USAGE, SELECT ON ALL SEQUENCES IN SCHEMA public TO audit_admin;

-- 建立稽核日誌查詢角色（唯讀）
CREATE ROLE audit_reader;
GRANT SELECT ON audit_logs, audit_event_types, audit_logs_summary, audit_logs_public TO audit_reader;

-- 建立應用程式角色（只能寫入稽核日誌）
CREATE ROLE audit_writer;
GRANT INSERT ON audit_logs, audit_change_details TO audit_writer;
GRANT SELECT ON audit_event_types TO audit_writer;
