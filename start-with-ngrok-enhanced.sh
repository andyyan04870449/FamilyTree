    ...
    while [ $RETRY_COUNT -lt $MAX_RETRIES ]; do
        ...
        if [ ! -z "$TUNNELS_RESPONSE" ]; then
            ...
            if command -v jq &> /dev/null; then
                FRONTEND_URL=$(echo "$TUNNELS_RESPONSE" | jq -r '.tunnels[] | select(.config.addr | contains("'${FRONTEND_PORT}'")) | .public_url' 2>/dev/null | head -1)
                BACKEND_URL=$(echo "$TUNNELS_RESPONSE" | jq -r '.tunnels[] | select(.config.addr | contains("'${BACKEND_PORT}'")) | .public_url' 2>/dev/null | head -1)
            else
                # 備用方案：使用 grep 和 sed
                FRONTEND_URL=$(echo "$TUNNELS_RESPONSE" | grep -o '"public_url":"[^"]*"' | grep "${FRONTEND_PORT}" | sed 's/"public_url":"//g' | sed 's/"//g' | head -1)
                BACKEND_URL=$(echo "$TUNNELS_RESPONSE" | grep -o '"public_url":"[^"]*"' | grep "${BACKEND_PORT}" | sed 's/"public_url":"//g' | sed 's/"//g' | head -1)
            fi
